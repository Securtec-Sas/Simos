...
class CryptoArbitrageV3:
    ...
    async def _on_get_ai_model_details_request(self):
        ...
        try:
            print(f"🤖 Solicitud de detalles del modelo de IA recibida desde UI")
            self.logger.info("Solicitud de detalles del modelo de IA recibida desde UI")
            
            # Obtener información del modelo
            model_info = self.ai_model.get_model_info()
            print(f"📊 Información del modelo obtenida: {model_info}")
            
            # Enviar directamente a través del broadcaster
            await self.ui_broadcaster.broadcast_message({
                "type": "ai_model_details",
                "payload": model_info
            })
            
            print(f"✅ Detalles del modelo enviados a la UI")
            
        except Exception as e:
            print(f"❌ Error obteniendo detalles del modelo de IA: {e}")
