...
class AdvancedSimulationEngine:
    ...
    async def _prepare_ai_input_data(self, symbol_dict: Dict, opportunity_data: Dict) -> Dict:
        """Prepara los datos de entrada para la IA obteniendo información desde endpoints de Sebo."""
        try:
            ...
            # Obtener precios actuales desde endpoints de Sebo
            buy_exchange_id = symbol_dict['buy_exchange_id']
            sell_exchange_id = symbol_dict['sell_exchange_id']
            symbol = symbol_dict['symbol']
            
            # Obtener información de mercado desde Sebo
            market_info = await self._get_market_info_from_sebo(buy_exchange_id, sell_exchange_id, symbol)
            
            # Obtener balances desde Sebo
            balance_info = await self._get_balance_info_from_sebo(buy_exchange_id)
            
            # Usar precios del socket si están disponibles, sino usar los de Sebo
            ...
            if buy_price == 0 and market_info.get('buy_price'):
                buy_price = market_info['buy_price']
            if sell_price == 0 and market_info.get('sell_price'):
                sell_price = market_info['sell_price']
            
            # Usar fees reales desde Sebo
            estimated_buy_fee = market_info.get('buy_fee', 0.001)
            estimated_sell_fee = market_info.get('sell_fee', 0.001)
            estimated_transfer_fee = market_info.get('transfer_fee', 1.0)
            
            # Usar balance real si está disponible
            if balance_info.get('usdt_balance'):
                current_balance = balance_info['usdt_balance']
                investment_amount = min(current_balance * (investment_percentage / 100), 100.0)
            
            return {
                'symbol': symbol,
                'symbol_name': symbol_dict.get('symbol_name', symbol),
                ...
                'balance_config': balance_info,
                'timestamp': get_current_timestamp()
            }
            
        except Exception as e:
            self.logger.error(f"Error preparando datos AI desde Sebo: {e}")
            # Fallback a datos básicos
            return await self._prepare_ai_input_data_fallback(symbol_dict, opportunity_data)
    
    async def _get_market_info_from_sebo(self, buy_exchange_id: str, sell_exchange_id: str, symbol: str) -> Dict:
        """Obtiene información de mercado desde endpoints de Sebo."""
        try:
            market_info = {
                'buy_price': 0,
                'sell_price': 0,
            ...
            # Obtener ticker del exchange de compra
            buy_ticker_url = f"{SEBO_API_BASE_URL}/api/local-operations/ticker"
            buy_ticker_params = {'exchangeId': buy_exchange_id, 'symbol': symbol}
            
            buy_ticker_result = await make_http_request(
                self.http_session,
                'GET',
                buy_ticker_url,
                params=buy_ticker_params,
                timeout=10
            )
            
            if buy_ticker_result and buy_ticker_result.get('success'):
                ticker_data = buy_ticker_result.get('data', {})
                market_info['buy_price'] = safe_float(ticker_data.get('ask', 0))
                market_info['buy_fee'] = safe_float(ticker_data.get('taker_fee', 0.001))
            
            # Obtener ticker del exchange de venta
            sell_ticker_url = f"{SEBO_API_BASE_URL}/api/local-operations/ticker"
            sell_ticker_params = {'exchangeId': sell_exchange_id, 'symbol': symbol}
            
            sell_ticker_result = await make_http_request(
                self.http_session,
                'GET',
                sell_ticker_url,
                params=sell_ticker_params,
                timeout=10
            )
            
            if sell_ticker_result and sell_ticker_result.get('success'):
                ticker_data = sell_ticker_result.get('data', {})
                market_info['sell_price'] = safe_float(ticker_data.get('bid', 0))
                market_info['sell_fee'] = safe_float(ticker_data.get('taker_fee', 0.001))
            
            # Obtener información de redes para transfer fee
            networks_url = f"{SEBO_API_BASE_URL}/api/local-operations/networks"
            networks_params = {'exchangeId': buy_exchange_id, 'symbol': symbol}
            
            networks_result = await make_http_request(
                self.http_session,
                'GET',
                networks_url,
                params=networks_params,
                timeout=10
            )
            
            if networks_result and networks_result.get('success'):
                networks_data = networks_result.get('data', {})
                # Buscar la red más barata
                min_fee = float('inf')
                for network_name, network_info in networks_data.items():
                    if network_info.get('withdraw') and network_info.get('fee'):
                        fee = safe_float(network_info['fee'])
                        if fee < min_fee:
                            min_fee = fee
                
                if min_fee != float('inf'):
                    market_info['transfer_fee'] = min_fee
            
            return market_info
            
        except Exception as e:
            self.logger.error(f"Error obteniendo info de mercado desde Sebo: {e}")
            return {
                'buy_price': 0,
                'sell_price': 0,
                'buy_fee': 0.001,
    ...
    async def _get_balance_info_from_sebo(self, exchange_id: str) -> Dict:
        """Obtiene información de balance desde endpoints de Sebo."""
        try:
            balance_url = f"{SEBO_API_BASE_URL}/api/local-operations/balance"
            balance_params = {'exchangeId': exchange_id}
            
            balance_result = await make_http_request(
                self.http_session,
                'GET',
                balance_url,
            ...
            if balance_result and balance_result.get('success'):
                ...
                return {
                    'balance_usdt': safe_float(usdt_info.get('free', 0)),
                    'reserved_balance': safe_float(usdt_info.get('used', 0)),
                    'total_balance': safe_float(usdt_info.get('total', 0))
                }
            
            return {}
            
        except Exception as e:
            self.logger.error(f"Error obteniendo balance desde Sebo: {e}")
            return {}
    
    async def _prepare_ai_input_data_fallback(self, symbol_dict: Dict, opportunity_data: Dict) -> Dict:
        """Fallback para preparar datos AI cuando Sebo no está disponible."""
        # Calcular monto de inversión basado en balance actual
        current_balance = self.simulation_stats['current_balance']
        investment_percentage = 10.0  # 10% del balance
        investment_amount = min(current_balance * (investment_percentage / 100), 100.0)
...
async def _call_sebo_local_api(self, endpoint: str, data: Dict) -> Dict:
        """Llama a la API de operaciones locales de Sebo."""
        try:
            url = f"{SEBO_API_BASE_URL}/api/local-operations/{endpoint}"
            ...
            if result:
                self.logger.debug(f"API local operations call successful: {endpoint}")
                return result
            else:
                return {
                    'success': False,
                    'message': f'Error calling local operations API: {endpoint}'
                }
                
        except Exception as e:
            self.logger.error(f"Error calling local operations API {endpoint}: {e}")
            return {
                'success': False,
                'message': str(e)
            }
