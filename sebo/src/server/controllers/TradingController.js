...
const createTrainingCSV = async (req, res) => {
  try {
    ...
    for (const analysis of analysisList) {
      ...
      const lowestFeeResult = await getLowestFeeNetwork(data.buyExchangeId, data.sellExchangeId, data.symbol);

      if (await lowestFeeResult.error) {
        console.warn(`Advertencia: No se pudo obtener la red de menor comisión para el símbolo ${data.symbol}: ${lowestFeeResult.error}`);
      } else if (Array.isArray(lowestFeeResult) && lowestFeeResult.length > 0) {
        // Asumiendo que getLowestFeeNetwork devuelve un array. Tomamos el primer elemento.
        data.transferFee = lowestFeeResult[0].commission;
        data.network = lowestFeeResult[0].network;
      } else {
