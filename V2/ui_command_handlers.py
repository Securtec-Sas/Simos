...
from controllera.testController import test_controller # Import test controller
...
class UICommandHandlers:
    ...
    async def handle_ui_message(self, websocket_client, message_json: str):
        try:
            ...
            elif msg_type == "create_test_file":
                asyncio.create_task(self.trigger_create_test_file(payload, websocket_client))
            elif msg_type == "get_test_files":
                asyncio.create_task(self.get_test_files_list(payload, websocket_client))
            else:
    ...
    async def trigger_create_test_file(self, payload: dict, websocket_client):
        """Maneja la creación de archivos de prueba."""
        client_id = id(websocket_client)
        
        try:
            # Extraer parámetros del payload
            days_back = payload.get("days_back", 59)
            num_analysis = payload.get("num_analysis", 100)
            frame_time = payload.get("frame_time", "5m")
            
            print(f"UICommandHandlers: Creando archivo de pruebas - {days_back} días, {num_analysis} análisis, {frame_time}")
            
            # Notificar inicio
            await self.app.broadcast_to_ui({
                "type": "test_file_creation_update",
                "payload": {
                    "status": "starting",
                    "client_id": client_id,
                    "parameters": {
            ...
            if result["success"]:
                ...
                print(f"UICommandHandlers: Archivo de pruebas creado exitosamente: {result['filename']}")
            else:
                # Notificar error
                await self.app.broadcast_to_ui({
                    "type": "test_file_creation_update",
                ...
                print(f"UICommandHandlers: Error creando archivo de pruebas: {result['message']}")
                
        except Exception as e:
            error_msg = f"Error en trigger_create_test_file: {str(e)}"
            print(f"UICommandHandlers: {error_msg}")
            await self.app.broadcast_to_ui({
                "type": "test_file_creation_update",
                "payload": {
                    "status": "failed",
    ...
    async def get_test_files_list(self, payload: dict, websocket_client):
        """Obtiene la lista de archivos de prueba disponibles."""
        client_id = id(websocket_client)
        
        try:
            print("UICommandHandlers: Obteniendo lista de archivos de prueba")
            ...
            print(f"UICommandHandlers: Lista de archivos de prueba enviada - {result.get('count', 0)} archivos")
                
        except Exception as e:
            error_msg = f"Error en get_test_files_list: {str(e)}"
            print(f"UICommandHandlers: {error_msg}")
            await websocket_client.send(json.dumps({
                "type": "test_files_list_response",
                "payload": {
