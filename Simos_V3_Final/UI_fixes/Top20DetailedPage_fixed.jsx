// UI/clients/src/components/Top20DetailedPage/Top20DetailedPage.jsx - VERSIÓN MEJORADA

import React, { useState, useEffect } from 'react';
import io from 'socket.io-client';

const SEBO_WEBSOCKET_URL = 'ws://localhost:3031';
const NAMESPACE = '/api/spot/arb';

const Top20DetailedPage = ({ sendV3Command, v3Data }) => {
  ...
  useEffect(() => {
    ...
    const connectToSebo = () => {
      try {
        socket = io(`${SEBO_WEBSOCKET_URL}${NAMESPACE}`, {
          transports: ['websocket'],
          timeout: 5000
        });

        socket.on('connect', () => {
          console.log('Connected to Sebo WebSocket (Top 20 Detailed)');
        ...
        socket.on('disconnect', (reason) => {
          console.log('Disconnected from Sebo WebSocket:', reason);
          setConnectionStatus('disconnected');
          
          if (reason !== 'io client disconnect' && reconnectAttempts < maxReconnectAttempts) {
            reconnectAttempts++;
            console.log(`Attempting to reconnect (${reconnectAttempts}/${maxReconnectAttempts})...`);
            reconnectTimeout = setTimeout(connectToSebo, 3000 * reconnectAttempts);
          }
        });

        socket.on('connect_error', (err) => {
          console.error('Connection error with Sebo WebSocket:', err);
          setConnectionStatus('error');
          setError(`Error de conexión: ${err.message}. ¿Está Sebo ejecutándose en el puerto 3031?`);
        });
        ...
        console.error('Error creating Sebo socket:', error);
        setConnectionStatus('error');
        setError(`Error creando conexión: ${error.message}`);
      }
    };
  ...
  // Monitorear estado de trading desde V3
  useEffect(() => {
    ...
    <div style={containerStyle}>
      ...
      <div style={headerStyle}>
        ...
        <div style={statusStyle}>
          <div style={statusBadgeStyle(connectionStatus)}>
            Sebo: {connectionStatus}
          </div>
      ...
      {opportunities.length === 0 ? (
        <div style={{
          ...
          <h3>Esperando oportunidades de arbitraje...</h3>
          <p>Conectando con Sebo para recibir datos en tiempo real</p>
        </div>
      ) : (
